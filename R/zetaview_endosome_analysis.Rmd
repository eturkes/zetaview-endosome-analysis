...
packages <- c("conflicted", "RColorBrewer", "reshape2", "ggplot2", "MASS", "jtools")
...
```{r}
summ(model, robust = TRUE, model.fit = FALSE)
effect_plot(model, pred = group, plot.points = TRUE, interval = TRUE)
effect_plot(model, pred = sex, plot.points = TRUE, interval = TRUE)
plot_summs(model)
```
